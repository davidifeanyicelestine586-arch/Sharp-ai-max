...
`;
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-fade-in text-slate-100">
      {/* Page Header */}
      <PageHeader
        kicker="MULTI-CHANNEL ENGINE · 5 FORMATS SYNCHRONIZED"
        title="Content Stacker"
        description="Input one core concept to simultaneously generate five publication-ready deliverables: an SEO blog post, LinkedIn narrative, X thread, Instagram caption, and newsletter broadcast."
      />
      ...
      <Card variant="default" padding="lg" className="space-y-4 shadow-sm">
        <FormField
          id="stacker-idea-box"
          label="Core Idea, Thesis, Product Release, or Outline"
          rightNote={`${ideaInput.length}/4000 characters`}
          description={
        ...
        <div className="flex items-center justify-end pt-1">
          <Button
            ...
            onClick={handleStack}
            disabled={isStacking}
            leftIcon={!isStacking ? <Layers className="h-4 w-4" /> : undefined}
          >
            {isStacking ? loadingStep : 'Generate 5-in-1 Stack'}
          </Button>
        </div>
      </Card>
      ...
      {stackedAssets && channelStats && (
        <div className="space-y-6 animate-fade-in">
          {/* Executive Channel Summary Card */}
          <Card variant="subtle" padding="md" className="border-indigo-500/30 bg-indigo-950/20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 items-start">
            ...
            <div className="pt-3 border-t border-slate-800 flex flex-col gap-2">
              ...
              >
                {isSaved ? 'Suite Saved to Studio' : 'Save Suite to Studio'}
              </Button>
              ...
              <Button
                ...
                fullWidth
                onClick={handleCopyAll}
                leftIcon={copiedChannel === 'all' ? <Check className="h-4 w-4 text-emerald-400" /> : <Copy className="h-4 w-4" />}
              >
                {copiedChannel === 'all' ? 'All Channels Copied' : 'Copy All 5 Channels'}
              </Button>
            </div>
            </Card>

          {/* Workstation Output Display */}
          <Card variant="default" padding="lg" className="lg:col-span-8 space-y-4 shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div>
              ...
              <Button
                ...
                leftIcon={copiedChannel === activeWorkspaceTab ? <Check className="h-3.5 w-3.5 text-emerald-400" /> : <Copy className="h-3.5 w-3.5" />}
                aria-label={`Copy ${activeWorkspaceTab} text`}
              >
                {copiedChannel === activeWorkspaceTab ? 'Copied' : 'Copy Channel'}
              </Button>
            </div>
        ...
        >
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Button
              ...
              size="xs"
              onClick={handleCopyAll}
              leftIcon={<Copy className="h-3.5 w-3.5 text-slate-400" />}
            >
              {copiedChannel === 'all' ? 'Copied All' : 'Copy All'}
            </Button>
            <Button
              variant="primary"
              size="xs"
